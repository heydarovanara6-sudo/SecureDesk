  ...
  return (
    <React.Fragment>
      <button onClick={() => setOpen(p => !p)}
        className="fixed right-4 z-50 w-12 h-12 rounded-full flex items-center justify-center transition-all duration-300"
        style={{ bottom: 'calc(70px + env(safe-area-inset-bottom))', background: 'linear-gradient(135deg,#007A3D,#00A650)', boxShadow: '0 4px 24px rgba(0,166,80,0.4)', transform: open ? 'scale(0.9)' : 'scale(1)' }}>
        {unread > 0 && !open && (
      ...
      {open && (
        <div className="fixed right-4 z-50 w-80 rounded-2xl flex flex-col overflow-hidden scale-in"
          style={{ bottom: 'calc(130px + env(safe-area-inset-bottom))', background:'var(--surface-1)', border:'1px solid var(--border)', boxShadow:'0 16px 48px rgba(0,0,0,0.6)', maxHeight:'450px' }}>
